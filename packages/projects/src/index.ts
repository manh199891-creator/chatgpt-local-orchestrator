...
export interface ProjectCommandDefinition { id: string; executable: string; args: string[]; timeoutSeconds: number; }
...
export class ProjectRegistry {
  ...
  async registerProject(input: ProjectInput): Promise<ProjectDefinition> { validateProjectInput(input, this.allowedProjectRoots); return this.withLock(async () => { const path = this.file(input.projectId); try { await access(path); throw new ProjectError("PROJECT_ALREADY_EXISTS"); } catch (e) { if (e instanceof ProjectError) throw e; if ((e as NodeJS.ErrnoException).code !== "ENOENT") storageError("PROJECT_STORAGE_WRITE_FAILED", e); } const now = new Date().toISOString(), project = { schemaVersion: 1 as const, ...input, repositoryPath: resolve(input.repositoryPath), createdAt: now, updatedAt: now }; await this.atomicWrite(path, project); return project; }); }
  async updateProject(projectId: string, input: Omit<ProjectInput, "projectId">): Promise<ProjectDefinition> { validateProjectId(projectId); validateProjectInput({ ...input, projectId }, this.allowedProjectRoots); return this.withLock(async () => { const old = await this.getProject(projectId), project = { ...input, projectId, schemaVersion: 1 as const, repositoryPath: resolve(input.repositoryPath), createdAt: old.createdAt, updatedAt: new Date().toISOString() }; await this.atomicWrite(this.file(projectId), project); return project; }); }
