...
import { mkdtemp, rm, writeFile, mkdir, watch } from 'node:fs/promises';
...
async function makeApp(token:string|null='secret-token',register=false){
  ...
  const a=buildBridgeApp({runtimeRootDirectory:root,authToken:token ?? undefined,allowedProjectRoots:[root],generateJobId:()=> 'JOB-00000000-0000-0000-0000-000000000001'}); apps.push(a);
...
describe('bridge lifecycle',()=>{
 ...
 describe('workflow API auth and validation',()=>{
  const workflow={workflowVersion:1,projectId:'project',goal:'Validate workflow route safely',tasks:[{taskId:'implementation',agentType:'CODEX',instruction:'Do not execute in this validation test.',dependsOn:[]}]};
  it('requires the existing bearer auth for every workflow route',async()=>{const {a}=await makeApp();for(const request of [{method:'POST',url:'/api/workflows',payload:workflow},{method:'GET',url:'/api/workflows/workflow-missing'},{method:'POST',url:'/api/workflows/workflow-missing/cancel'}]){expect((await a.inject(request as any)).statusCode).toBe(401);expect((await a.inject({...request,headers:{authorization:'Bearer invalid'}} as any)).statusCode).toBe(401)}});
  it('returns safe structured errors before workflow execution for invalid, unknown-project, and unsafe-command plans',async()=>{const {a}=await setup();const invalid=await a.inject({method:'POST',url:'/api/workflows',headers:h,payload:{...workflow,workflowVersion:2}});expect(invalid.statusCode).toBe(400);expect(invalid.json().error.code).toBe('WORKFLOW_INVALID');const missing=await a.inject({method:'POST',url:'/api/workflows',headers:h,payload:{...workflow,projectId:'missing-project'}});expect(missing.statusCode).toBe(404);const unsafe=await a.inject({method:'POST',url:'/api/workflows',headers:h,payload:{...workflow,executable:'powershell.exe'}});expect(unsafe.statusCode).toBe(400);expect(unsafe.json().error.code).toBe('WORKFLOW_INVALID')});
 it('returns workflow get and cancel not-found responses without leaking runtime data',async()=>{const {a}=await makeApp();for(const request of [{method:'GET',url:'/api/workflows/no-such-workflow'},{method:'POST',url:'/api/workflows/no-such-workflow/cancel'}]){const response=await a.inject({...request,headers:h});expect(response.statusCode).toBe(404);expect(JSON.stringify(response.json())).not.toMatch(/token|process\.env|stdout|stderr/i)}});
  it('acknowledges durable workflow identity promptly and makes response-loss retry idempotent',async()=>{const {a,root}=await makeApp('secret-token',false),repo=await createTemporaryGitRepository(root);await a.inject({method:'POST',url:'/api/projects',headers:h,payload:{...project(repo),projectId:'prompt-workflow-project',commands:[{id:'slow',executable:process.execPath,args:['-e','setTimeout(()=>{},3000)'],timeoutSeconds:30,agentTypes:['CODEX']}]}});const payload={workflowVersion:1,projectId:'prompt-workflow-project',goal:'Acknowledge durable identity before long execution.',tasks:[{taskId:'slow',agentType:'CODEX',instruction:'Remain active long enough to prove prompt acknowledgement.',dependsOn:[]}]},headers={...h,'x-idempotency-key':'bridge-response-loss-retry'},started=Date.now(),first=await a.inject({method:'POST',url:'/api/workflows',headers,payload});expect(first.statusCode,first.body).toBe(201);expect(Date.now()-started).toBeLessThan(2000);expect(first.json().data.status).toBe('ACCEPTED');const second=await a.inject({method:'POST',url:'/api/workflows',headers,payload});expect(second.statusCode,second.body).toBe(201);expect(second.json().data.workflowId).toBe(first.json().data.workflowId);expect((await (await import('node:fs/promises')).readdir(join(root,'workflows'))).filter(id=>id===first.json().data.workflowId)).toHaveLength(1);await a.inject({method:'POST',url:`/api/workflows/${first.json().data.workflowId}/cancel`,headers:h})});
  it('cancels a live authenticated workflow through the bridge without a CLI agent',async()=>{const {a,root}=await makeApp('secret-token',false);const repo=await createTemporaryGitRepository(root),startedPath=join(root,'runner-started'),events=watch(root);const waitForRunnerStart=(async()=>{for await(const event of events)if(event.filename==='runner-started'){await events.return?.();return}})();const registration=await a.inject({method:'POST',url:'/api/projects',headers:h,payload:{...project(repo),projectId:'workflow-project',commands:[{id:'hold',executable:process.execPath,args:['-e',"require('node:fs').writeFileSync(process.argv[1],'started');setInterval(()=>{},1000)",startedPath],timeoutSeconds:30,agentTypes:['CODEX']}]}});expect(registration.statusCode).toBe(201);const live={workflowVersion:1,workflowId:'workflow-live-cancel',projectId:'workflow-project',goal:'Cancel a controlled local workflow process.',tasks:[{taskId:'hold',agentType:'CODEX',instruction:'Remain active until cancellation.',dependsOn:[]}]};const submitted=await a.inject({method:'POST',url:'/api/workflows',headers:h,payload:live});expect(submitted.statusCode,submitted.body).toBe(201);await waitForRunnerStart;const running=await a.inject({method:'GET',url:'/api/workflows/workflow-live-cancel',headers:h});expect(running.json().data).toMatchObject({status:'RUNNING',currentTaskId:'hold'});const cancelled=await a.inject({method:'POST',url:'/api/workflows/workflow-live-cancel/cancel',headers:h});expect(cancelled.statusCode).toBe(200);expect(cancelled.json().data.status).toBe('CANCELLED');const repeated=await a.inject({method:'POST',url:'/api/workflows/workflow-live-cancel/cancel',headers:h});expect(repeated.json().data.status).toBe('CANCELLED');const persisted=JSON.parse(await (await import('node:fs/promises')).readFile(join(root,'workflows','workflow-live-cancel','workflow-state.json'),'utf8'));expect(persisted.status).toBe('CANCELLED');});
 });
});
